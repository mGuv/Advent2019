...
namespace Advent2019.Problems
{
    public class Day6 : IProblem
    {
        ...
        public Task RunPart1Async(CancellationToken cancellationToken)
        {
            ...
            {
                string parent = keyValuePair.Value;
                while (parent != "")
                {
                    count++;
        ...
        public Task RunPart2Async(CancellationToken cancellationToken)
        {
           Console.WriteLine("Please enter your orbit map int x)y:");
            ...
            // go up parent stack till we find the same
            Dictionary<string,int> planets = new Dictionary<string, int>();
            string myParent = map["YOU"];
            int jumps = 0;
            while (myParent != null)
            {
                planets.Add(myParent, jumps);
                if (myParent != "COM")
                {
            ...
            string santaParent = map["SAN"];
            jumps = 0;
            while (!planets.ContainsKey(santaParent))
            {
                if (santaParent != "COM")
                {
            ...
            jumps += planets[santaParent];

            Console.WriteLine($"{jumps} jumps to santa");
